...
class Goblin extends Creature {
    ...
    public int getAttack() {
        ...
        for (Creature c : game.creatures) c.query(this, q);
        return q.result;
    }
    ...
    public void query(Object source, StatQuery sq) {
        if (source == this) sq.result += sq.statistic == Statistic.ATTACK ? baseAttack : baseDefense;
        else if (sq.statistic == Statistic.DEFENSE) sq.result++;
    }
}
...
class GoblinKing extends Goblin {
    ...
    @Override
    public void query(Object source, StatQuery sq) {
        if (source != this && sq.statistic == Statistic.ATTACK) sq.result++; // every goblin gets +1 attack
